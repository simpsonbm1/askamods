...
## Per-frame work throttling (v1.5.5 — confirmed in-game 2026-07-07)

Well-refill and respawn-queue servicing tasks in `DayTracker.Update()` gated to 1 Hz. World-poll (checking `ActiveSessionID` for world changes), mushroom availability, and hotkeys remain per-frame.
...
### v1.5.7/v1.5.8 — race post-mortem, corrected ground truth, census verification (2026-07-10/11)

**v1.5.7 (2026-07-10) — convergent availability re-sweep (hardening).** `MushroomAvailability.MaybeApply()` no longer latches after one sweep: first sweep unchanged (~5 s settle after descriptors appear), then re-sweeps every 10 s for the whole session. Any late-caught entry logs an unconditional `LATE clear:` line (fire-verification marker). Originals are snapshotted only from a populated read (`seasonBefore+otherBefore > 0`) so a raced-empty read can never be recorded as vanilla data. All per-item log lines now carry `(id N)`. **NOTE:** zero `LATE clear` lines fired across two long test sessions on the dev machine — the race never manifested locally; kept as hardening because it's plausible on slower machines (and is a live hypothesis for a Nexus user report, tomkat2351).

**CORRECTION of the v1.4.7-era research claim:** plain "Mushrooms" (id 16793608) is **vanilla-UNGATED** in the weather availability table — both Season and Other condition lists are empty in a fresh process, deterministically (confirmed in-game 2026-07-10 and 2026-07-11, two sessions). The old idea-9 research line pairing it with Grey's gates was a summarization conflation. **Actual vanilla gating:** Grey Mushrooms (id 16793609): Season[Spring,Summer,Autumn] + Other[IsRaining mandatory]; Yellow Mushrooms (id 16793610): Season[Autumn] + Other[IsRaining mandatory]; plain Mushrooms (id 16793608): ungated.

**Grey↔Gray plant/yield pairing (confirmed in-game 2026-07-11):** the weather table registers the PLANT ItemInfo named "Grey Mushrooms"; the world gather nodes' YIELD is a distinct ItemInfo named "Gray Mushrooms". Census instance counts (Yellow 9 / Grey 6 / plain 1) exactly matched the mod's KnownNodes gather tallies ("Yellow Mushrooms"=9, "Gray Mushrooms"=6, "Mushrooms"=1). Both spellings are raw dev-typed `ItemInfo.Name` strings (en-GB vs en-US in one game), NOT localization-resolved — so name filters are locale-safe as far as observed, but plant vs yield items can be DIFFERENT ItemInfos with different names.

**v1.5.8 (2026-07-11) — read-only instance census in the F8 dump.** `MushroomDiag.Dump()` now appends per mushroom entry a `census:` line — walks `BiomeItemAvailabilityData.itemDescriptors` (interop List: Count + indexer) → per `BiomeItemDescriptor` reads `IsAvailable`/`IsHarvestable`/`._instances` → per `BiomeItemInstance` reads `.Active`/`.Destroyed`/`.GetQuantity()` — reporting instances/active/destroyed/stocked/qtySum plus a per-descriptor tail. Also logs a `KnownNodes` mushroom-gather tally line per dump. Instance lists reflect the currently-streamed region.

**Two-soak in-game verification (2026-07-11, TimeWarp-accelerated, no rain throughout):**
- Soak #1 (idle): three F8 dumps across ~6 in-game days (dayOfYear 76→82) — census IDENTICAL at all three points (Yellow 9 instances/qty 27, Grey 6/6, plain 1/3). Zero attrition, zero culls with gates cleared.
- Soak #2 (forced consumption — gathering-station priority maxed on mushrooms): 30 mushroom exhaustion registrations + 24 mod respawns (0.1-day thresholds, 0.10–0.12 d verified elapsed; the ~6 unmatched were still pending at session end and persisted to the .save — reload-safe). Census read full strength at every dump. Zero errors both soaks.

**`remainingDays` lifecycle (confirmed in-game 2026-07-11):** `BiomeItemAvailabilityData.remainingDays` arms to the process `Lifespan` (observed 1) when availability is evaluated (at load / after the clear), decrements per day, then PARKS at −1 — and does NOT re-arm from instance consumption (stayed −1 across a forced-harvest soak). With gates permanently cleared there are no availability transitions, so the game's own replenish countdown goes dormant after the first cycle. **Implication: an availability un-gate must be paired with a node-respawn mechanism — in TreeRespawnMod, the existing `[GatherRespawn]` machinery is the restore engine for harvested mushroom nodes (Default fallback covers mushrooms out of the box).** Working model: availability edit = stops culls/unavailability; gather-respawn = restores harvested nodes.

**Minor observations:** plain-"Mushrooms" nodes are rare (1 instance in the streamed home region, qty 3). `AvailabilityProcess.CanRun` reads False when all condition lists are empty (seen on both cleared and vanilla-empty processes) — ⚠️ semantics unconfirmed, observation only. The weather descriptor table held 22 entries in the test world (mushrooms + crops/berries/driftwood/nests/etc.).
