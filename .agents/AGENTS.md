...
askamods/
  ...
  DynamicVillagerNeedsMod/   ← Mod 5: needs-based villager behavior         [COMPLETE v1.9.5 — typing guard (BuilderDiag F9 guarded for consistency), confirmed in-game 2026-07-10; shift-scheduling QoL round (v1.9.x, confirmed in-game 2026-07-10): per-station off-duty fill via `ManualScheduleStations` `Name:Fill` syntax (first-match-wins substrings); three-brush semantic (painted Work = post, painted Leisure = honored as painted, painted Sleep = flexible time the mod optimizes; brush is the toggle, no config); station-name discovery file `com.askamods.dynamicneeds.stations.txt` + non-diagnostic mismatch feedback (unmatched-cohort INFO, dead-entry WARNING); live cfg reload every 5 s; rename-proof matching (display name OR `Structure.DefaultName`; DefaultName pristine after player renames); diagnostics defaults false (pre-Nexus); builder-loan lend/restore (v1.8.0, 227 cycles verified); `PreserveScheduleInSaves`/`ShowIntendedScheduleInUI` (v1.7.0–v1.8.0); universal player-edit adoption; per-frame throttled — docs/mods/dynamic-villager-needs.md]
  ...
  GroundItemVacuumMod/       ← Mod 19: hotkey/auto vacuum for loose ground items [COMPLETE v1.1.1 — typing guard (vacuum hotkey ignored while a text field is focused), confirmed in-game 2026-07-10; v1.1.0 live config hot-reload (cfg re-read every 5 s in-session, hotkey re-binds on change; SeedScout pattern), confirmed in-game 2026-07-08; v1.0.2 default hotkey changed from V to N (V conflicted with emote wheel), confirmed in-game 2026-07-08. Removes debris cleanly (own-set OnEnable/OnDisable tracking + RemoveObjectFromWorld), no crash. Ground clutter was NOT the framerate bottleneck (a loaded mod is — bisect pending) — docs/mods/ground-item-vacuum.md]
  ...
  VillagerAmmoMod/           ← Mod 24: villagers never run out of arrows (polling ammo-refund) [COMPLETE v0.2.1 — refund confirmed in-game 2026-07-11 (146 refunds/0 false adoptions); v0.2.1 ground-item cull for persisted arrows confirmed in-game 2026-07-11 (culled 2,533/2,533 arrows near 103 targets within ~60s, framerate recovered); v0.2.0 ReleaseAllStuckObjects cleanup ineffective (persisted arrows = DynamicItemObjects, registry empty after reload) — docs/mods/villager-ammo.md]
