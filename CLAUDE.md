...
| [`NEW_MOD_IDEAS_PLAN.md`](NEW_MOD_IDEAS_PLAN.md) | Researched mod ideas (2026-07-03, +7–8 on 2026-07-06, +10 on 2026-07-07, +11 on 2026-07-08, +12 on 2026-07-09, +13–14 on 2026-07-10): well refill (✔ shipped, TreeRespawn v1.4.x), den respawn (Den.Revive + structure-blocking insight), crafting multiplier (BlueprintInfo.quantity data edit), ground-item vacuum (DynamicItemObjectManager), freezing hunters (warmth objective thresholds), pre-construction worker/task setup (Structure.Activate + buildsite-aware WorkstationMenu), zero-task new workers (_CanAddVillagerToTaskData gate), inventory fish filleting (ResourceInfo.exhaustableComponents + ItemThumbnailPanel.CanHarvestCurrentItem gate), opt-in manual-schedule/coverage-staggering mode for DynamicVillagerNeeds (idea-11, Phase 2 `OffWindowFill=Builder` SHIPPED confirmed in-game 2026-07-10; Phase 3 schedule-UI overlap warning open/requested), demand-driven crafting prioritization (idea-12, SEPARATE-MOD, fully plausible — complete 2026-07-09 research: complaint events + task-priority machinery + phasing + unknowns), outhouse composter (idea-13, fully plausible — both item-decay and farm-compost systems are native, mod is plumbing), rocks-only remover (idea-14, hotkey+radius, core behavior already observed in-game 2026-07-01 as the bulldozer's `skipAllCollisionChecks` gotcha: clearItemInstances-only AOE cleared rocks and spared trees) — approaches + Cecil-confirmed API leads |
