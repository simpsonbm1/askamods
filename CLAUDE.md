...
askamods/
  ...
  DynamicVillagerNeedsMod/   ← Mod 5: needs-based villager behavior         [COMPLETE v1.8.0 — idea-11 Phase 2 `OffWindowFill=Builder` SHIPPED + confirmed in-game (2026-07-10): off-shift cohort villagers are LENT to the builder pool via the station-side agent registry (home `ReleaseTaskAgent` + Buildstation `SetTaskAgent`/`AddToTaskDatas` + all-Work schedule; `_workstation`/UI assignment never changes) and restored before shift start — 227 clean lend/restore cycles verified over multi-day 10x run; `PreserveScheduleInSaves` Harmony mask on `Villager.Serialize` fixes the quit-to-menu/autosave collapse-bake that destroyed painted schedules (confirmed in-game 2026-07-10); `ShowIntendedScheduleInUI` masks the schedule panel's reads (`VillagerSchedulePanel` Set/Refresh/OnEnable/HasUnappliedChanges) so the UI always shows the player's painted schedule, not the mod's temporary collapse (confirmed in-game 2026-07-10); universal mode-independent player-edit adoption (own-write discrimination via `_appliedPacked`); per-frame work throttled for framerate (confirmed in-game 2026-07-07); Buildstation family excluded from cohorts/manual/lending; `ManualScheduleStations` whitelist config — docs/mods/dynamic-villager-needs.md]
