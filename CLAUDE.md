...
askamods/
  ...
  DynamicVillagerNeedsMod/   ← Mod 5: needs-based villager behavior         [COMPLETE v1.5.0 — per-frame work throttled for framerate (confirmed in-game 2026-07-07); idea-11 Phase 1 RespectManualSchedule confirmed in-game 2026-07-09 (cook cohort; other stations assumed to track); v1.5.0 adds [DynamicNeeds] OffWindowFill enum (Leisure|Work|Builder, default Leisure), fill=Work confirmed in-game 2026-07-09; Builder deferred to diagnostics phase — docs/mods/dynamic-villager-needs.md]
...
| [`NEW_MOD_IDEAS_PLAN.md`](NEW_MOD_IDEAS_PLAN.md) | Researched mod ideas (2026-07-03, +7–8 on 2026-07-06, +10 on 2026-07-07, +11 on 2026-07-08, +12 on 2026-07-09): well refill (✔ shipped, TreeRespawn v1.4.x), den respawn (Den.Revive + structure-blocking insight), crafting multiplier (BlueprintInfo.quantity data edit), ground-item vacuum (DynamicItemObjectManager), freezing hunters (warmth objective thresholds), pre-construction worker/task setup (Structure.Activate + buildsite-aware WorkstationMenu), zero-task new workers (_CanAddVillagerToTaskData gate), inventory fish filleting (ResourceInfo.exhaustableComponents + ItemThumbnailPanel.CanHarvestCurrentItem gate), opt-in manual-schedule/coverage-staggering mode for DynamicVillagerNeeds (idea-11, Phase 1 confirmed in-game 2026-07-09; v1.5.0 OffWindowFill enum Leisure|Work|Builder, fill=Work confirmed; Builder diagnostics-gated; stretches: idle→leisure+concurrency-cap, runestone/outhouse investigation), demand-driven crafting prioritization (idea-12, SEPARATE-MOD, fully plausible — complete 2026-07-09 research: complaint events + task-priority machinery + phasing + unknowns) — approaches + Cecil-confirmed API leads |
