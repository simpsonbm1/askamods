...
askamods/
  ...
  TreeRespawnMod/            ← Mod 2: tree/gather respawn + constructed-well refill [COMPLETE v1.6.1 — perf hardening (v1.6.x, ⚠️ pending in-game confirmation): cheap-first due-check + cull-to-8-per-tick in tree loop; WellRefill.Tick moved to 30s cadence; ServiceInterval 1s→3s; respawn timing is day-gated so no latency impact); mushroom availability verified end-to-end (v1.5.7 convergent re-sweep hardening + v1.5.8 F8 instance census; two TimeWarp soaks incl. forced-harvest, confirmed in-game 2026-07-11; plain Mushrooms vanilla-ungated — earlier note corrected; game replenish countdown parks at −1 → the mod's own gather-respawn is the restore engine); typing guard (DayTracker hotkeys ignored while a text field is focused), confirmed in-game 2026-07-10; per-frame work throttled for framerate (confirmed in-game 2026-07-07); trees no longer respawn up through player-built structures: durable BlockedPositions set + reusable StructureQuery footprint check ([TreeRespawn] BlockRespawnUnderStructures/StructureBlockMargin), confirmed in-game 2026-07-07; mushrooms year-round + rain-independent ([MushroomAvailability] config) 2026-07-07; same-world reload crash fixed 2026-07-06; well refill 2026-07-04; co-op detection 2026-07-03 — docs/mods/tree-respawn.md + TREERESPAWN_HANDOFF.md]
  ...
  TorchFuelMod/              ← Mod 4: keep torches perpetually fueled       [COMPLETE v1.2.5 — [Perf] stopwatch on fuel-check pass; measured healthy (≤3.7 ms) — docs/mods/torch-fuel.md]
  DynamicVillagerNeedsMod/   ← Mod 5: needs-based villager behavior         [COMPLETE v1.9.7 — [Perf] sub-phase stopwatches (tick.cohorts, tick.villagers); RefreshCohorts already 10s-gated via _cohortTimer (avg 4.7 / max 28 ms); v1.9.6 cfg reload 5s→30s; v1.9.x typing guard + shift-scheduling QoL (confirmed in-game 2026-07-10): per-station off-duty fill via `ManualScheduleStations` `Name:Fill` syntax; three-brush semantic; station-name discovery file + non-diagnostic mismatch feedback; live cfg reload; rename-proof matching (display name OR `Structure.DefaultName`); builder-loan lend/restore (v1.8.0, 227 cycles verified); `PreserveScheduleInSaves`/`ShowIntendedScheduleInUI` (v1.7.0–v1.8.0); universal player-edit adoption; per-frame throttled — docs/mods/dynamic-villager-needs.md]
  ...
  MineRefreshMod/            ← Mod 11: safe, on-demand mine/cave refresh    [COMPLETE v1.3.2 — typing guard (trigger key ignored while a text field is focused), confirmed in-game 2026-07-10 — docs/mods/mine-refresh.md]
  ...
  GroundItemVacuumMod/       ← Mod 19: hotkey/auto vacuum for loose ground items [COMPLETE v1.1.2 — cfg reload 5s→30s (hotkey/config edits apply within 30s); typing guard (vacuum hotkey ignored while a text field is focused), confirmed in-game 2026-07-10; v1.1.0 live config hot-reload, confirmed in-game 2026-07-08; v1.0.2 default hotkey changed from V to N, confirmed in-game 2026-07-08. Removes debris cleanly (own-set tracking + RemoveObjectFromWorld), no crash. Ground clutter was NOT the framerate bottleneck (2026-07-11/12 bisect ran, perf arc is the continuation — see docs/architecture.md) — docs/mods/ground-item-vacuum.md]
  ...
  TimeWarpMod/               ← Mod 22: dev/test time accelerator (K=native fast-forward cycle to 162x, L=skip day) [DEV TOOL v0.1.1 — typing guard (K/L hotkeys ignored while a text field is focused), confirmed in-game 2026-07-10; confirmed in-game 2026-07-09; NOT for Nexus — docs/mods/time-warp.md]
  SummonTimerMod/            ← Mod 23: remove Eye of Odin villager-summon wait timer [COMPLETE v0.1.0 — confirmed in-game 2026-07-09; local-only, NOT for Nexus — docs/mods/summon-timer.md]
  VillagerAmmoMod/           ← Mod 24: villagers never run out of arrows (polling ammo-refund) [COMPLETE v0.2.3 — v0.2.3 cull centers filtered by `[TargetCleanup] TargetNameMatch` (default "ArcheryTarget,TrainingDummy"; case-insensitive substrings; census lines append `matched=`); TargetCleanup NameMatch + ReleaseAllStuckObjects sweep both enabled; v0.2.2 [Perf] poll/cleanup stopwatches + cfg reload 5s→30s + world-session target-helper census (79 villagers + 6 targets + 6 dummies + 5 skeletons + animals + harvestables = ProjectileTargetHelper rides on characters too); v0.2.1 ground-item cull confirmed in-game 2026-07-11 (2,533 arrows culled within ~60s, framerate recovered); fatal _OnAmmoRemoved patch dead-end documented — docs/mods/villager-ammo.md]
  OuthouseComposterMod/      ← Mod 25: outhouse composter — food/seeds convert to Compost inside the Outhouse storage on real-time ratio timers [WIP v0.2.0 — Phase 1 feature built 2026-07-11 ⚠️ PENDING in-game confirmation; Phase 0 read-only diagnostics confirmed in-game 2026-07-11 (see NEW_MOD_IDEAS_PLAN idea 13 for ground truth)]
```
...
|---|---|
| [`docs/architecture.md`](docs/architecture.md) | **Any** game subsystem — confirmed APIs + dead-ends, grouped: damage pipeline, player vs. creature, resource/tree, gather, structures/workstations, villager complaints/issue tracker, workstation task-priority mechanics (idea-12 groundwork), settlement hauling (Mod 6 groundwork), inventory/settlement/recipes, cooking station pipeline, torch/fire-fuel, villager needs/schedule/happiness, villager combat/fight-vs-flee, build menu/structure templates/localization, terrain/terraforming, dens/population spawners, villager summoning (Eye of Odin), native crash diagnosis (WER+Cpp2IL) |
| [`docs/mods/bow-damage.md`](docs/mods/bow-damage.md) | Mod 1 — BowDamageMod |
| [`docs/mods/tree-respawn.md`](docs/mods/tree-respawn.md) | Mod 2 — TreeRespawnMod (incl. the v1.4.x constructed-well refill recipe, the v1.4.7 mushroom year-round/rain-independent recipe + the v1.5.x block-respawn-under-structures recipe: durable BlockedPositions + StructureQuery footprint check + the v1.4.7–v1.5.8 mushroom availability recipe: corrected gating ground truth, census diagnostic, remainingDays lifecycle, Grey/Gray plant-vs-yield pairing) |
| [`NEW_MOD_IDEAS_PLAN.md`](NEW_MOD_IDEAS_PLAN.md) | Researched mod ideas (2026-07-03, +7–8 on 2026-07-06, +10 on 2026-07-07, +11 on 2026-07-08, +12 on 2026-07-09, +13–14 on 2026-07-10, +15 on 2026-07-11): well refill (✔ shipped, TreeRespawn v1.4.x), den respawn (Den.Revive + structure-blocking insight), crafting multiplier (BlueprintInfo.quantity data edit), ground-item vacuum (DynamicItemObjectManager), freezing hunters (warmth objective thresholds), pre-construction worker/task setup (Structure.Activate + buildsite-aware WorkstationMenu), zero-task new workers (_CanAddVillagerToTaskData gate), inventory fish filleting (ResourceInfo.exhaustableComponents + ItemThumbnailPanel.CanHarvestCurrentItem gate), opt-in manual-schedule/coverage-staggering mode for DynamicVillagerNeeds (idea-11, Phase 2 `OffWindowFill=Builder` SHIPPED confirmed in-game 2026-07-10; Phase 3 schedule-UI overlap warning open/requested), demand-driven crafting prioritization (idea-12, SEPARATE-MOD, fully plausible — complete 2026-07-09 research: complaint events + task-priority machinery + phasing + unknowns), outhouse composter (idea-13, fully plausible — both item-decay and farm-compost systems are native, mod is plumbing), rocks-only remover (idea-14, hotkey+radius, core behavior already observed in-game 2026-07-01 as the bulldozer's `skipAllCollisionChecks` gotcha: clearItemInstances-only AOE cleared rocks and spared trees), villager ammo refund (idea-15, SHIPPED as VillagerAmmoMod v0.1.3 polling approach, confirmed in-game 2026-07-11; recommended refill-on-remove approach 1 turned out to be fatal dead-end — any _OnAmmoRemoved patch crashes at load via inventory-family class-init) — approaches + Cecil-confirmed API leads |
...
| [`docs/mods/summon-timer.md`](docs/mods/summon-timer.md) | Mod 23 — SummonTimerMod (Eye of Odin summon timer removal; timer-scaling recipe via `_spawnTimeline.cooldowns` list + per-instance `gametimeToSpawnVillager`) |
| [`docs/mods/villager-ammo.md`](docs/mods/villager-ammo.md) | Mod 24 — VillagerAmmoMod (v0.1.3 polling ammo-refund: RangedManager.Awake capture + AmmoTracker.Update poller + grace window; v0.2.1 ground-item cull for persisted arrows: DynamicItemObject tracking + periodic sweep within target radius; v0.2.0 ReleaseAllStuckObjects dead-end: registry doesn't persist across save/load; fatal _OnAmmoRemoved patch dead-end with inventory-family class-init crash) |
