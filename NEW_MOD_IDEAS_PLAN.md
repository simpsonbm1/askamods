...
## 17. Craft from settlement storage — SHIPPED as CraftFromStorageMod v1.0.0 (confirmed in-game 2026-07-31)

Research retired. Both halves shipped, host/solo only: the player's just-in-time pull at the craft
click, and the villager station stocker that deletes the supply walk (including bloomery/kiln ore
delivery behind `StockTransformStationMaterials`). The full recipe, config reference, and every
dead-end (the `QuerySettlementResources()` hang, the structural equip-point probe, the fetch-quest
priority-suppression stall, the kiln-container drain) live in
[`docs/mods/craft-from-storage.md`](docs/mods/craft-from-storage.md) and `docs/architecture.md` →
Player crafting pipeline. The transform-station stand-aside design is in
`CraftFromStorageMod/TRANSFORM_STATION_PLAN.md`.
