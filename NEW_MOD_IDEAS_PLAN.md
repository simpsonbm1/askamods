...
- **Phase 2 QoL round — v1.9.0–v1.9.4 shipped (2026-07-10), shift-scheduling polish:**
  per-station off-duty fill via `ManualScheduleStations` `Name:Fill` syntax (first-match-wins substrings; Fill ∈ Leisure|Work|Builder); three-brush semantic (painted Leisure honored during that hour, painted Sleep = flexible fill time — no config, brush is the toggle); station-name discovery file `com.askamods.dynamicneeds.stations.txt` + non-diagnostic mismatch feedback (INFO for unmatched cohorts, WARNING for dead entries); live config reload every 5 s (`ConfigFile.Reload()` pattern); rename-proof matching (entries match display name OR `Structure.DefaultName`; DefaultName confirmed pristine after player renames, confirmed in-game 2026-07-10). Diagnostics defaults flipped false (pre-Nexus rule). See [`docs/mods/dynamic-villager-needs.md`](docs/mods/dynamic-villager-needs.md) for full recipe. Phase 3 (schedule-UI overlap warning) remains the next open feature.
