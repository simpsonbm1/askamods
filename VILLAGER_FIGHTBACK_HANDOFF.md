# Mod 7 — VillagerFightBackMod — Handoff (COMPLETE — v1.0.25 WORKING; verified in-game)
...
- **Safe Target Validity (v1.0.24):** Added a `Plugin.SafeIsAlive()` method with a local try-catch to wrap calls to `.IsAlive() && .IsViable()`. This prevents native exceptions when a dead target (like a destroyed Wisp) is garbage-collected or cleaned up, ensuring the combat timer is successfully cleared to `0f` and does not hang active.
- **Config Binding & JIT fix (v1.0.25):** Swapped out the custom `MockConfigEntry<T>` class for standard, JIT-friendly C# static properties (e.g. `Plugin.KeepCombatAlive`) to ensure BepInEx/CoreCLR correctly compiles and JITs the properties to `true` at runtime, restoring the top-up and early-exit timer loops.
...
**Status as of 2026-06-27:** Deployed **v1.0.25**, launched clean, and verified in-game. Villagers stand and fight Wisps and return to their jobs immediately after.
