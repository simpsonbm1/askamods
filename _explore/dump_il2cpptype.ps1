...
$asms = @()
foreach ($f in Get-ChildItem "$base\core\*.dll") {
...
$matches = $all | Where-Object { $_.Name -eq "Il2CppType" }
foreach ($t in $matches) {
    Write-Output "`n### $($t.FullName)"
    foreach ($m in ($t.Methods | Where-Object { $_.Name -eq "Of" })) {
        $ps = ($m.Parameters | ForEach-Object { "$($_.ParameterType.FullName) $($_.Name)" }) -join ", "
        Write-Output "  M $($m.Name)($ps) : $($m.ReturnType.FullName)  generic=$($m.HasGenericParameters)"
    }
}
