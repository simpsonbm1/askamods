...
function DumpType($tn) {
    ...
    foreach ($fld in $t.Fields) { if ($fld.Name -notmatch '^NativeF|^NativeM') { Write-Host "    F $($fld.Name) : $($fld.FieldType.FullName)" } }
    foreach ($p in $t.Properties) { Write-Host "    prop $($p.Name) : $($p.PropertyType.FullName)" }
    foreach ($m in $t.Methods) { if ($m.Name -notmatch "^(get_|set_|add_|remove_|\.c)") { Write-Host "    M $(Sig $m)" } }
    Write-Host ""
}
DumpType "SSSGame.Combat.PopulationSpawner/SpawnPopulation"
