...
foreach ($t in $asm.MainModule.Types) {
    if ($t.Name.Contains('Loot')) { Write-Output "SandSailorStudio: $($t.FullName)" }
    foreach ($nt in $t.NestedTypes) {
        if ($nt.Name.Contains('Loot')) { Write-Output "SandSailorStudio: $($nt.FullName)" }
    }
}
$asm2 = [Mono.Cecil.AssemblyDefinition]::ReadAssembly('D:\SteamLibrary\steamapps\common\ASKA\BepInEx\interop\Assembly-CSharp.dll', $rp);
foreach ($t in $asm2.MainModule.Types) {
    if ($t.Name.Contains('Loot')) { Write-Output "Assembly-CSharp: $($t.FullName)" }
    foreach ($nt in $t.NestedTypes) {
        if ($nt.Name.Contains('Loot')) { Write-Output "Assembly-CSharp: $($nt.FullName)" }
    }
}
